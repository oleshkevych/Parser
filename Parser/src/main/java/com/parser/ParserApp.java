...
import com.parser.parsers.com.jobs_justlanded.ParserJobsJustlanded;
...
import com.parser.parsers.io.unicornhunt.ParserUnicornhunt;
...
public class ParserApp implements MouseListener {
    ...
    private JPanel unicornhuntLabelPanel;
    private JLabel unicornhuntLabel;
    private JButton openSearchInSelected;
    private JPanel jobsJustlandedLabelPanel;
...
/*27*/
        mapParsers.put("jobs.remotive.io", new ParserJobsRemotive());
/*28*/
...
/*45*/
        mapParsers.put("unicornhunt.io", new ParserUnicornhunt());
/*46*/
        mapParsers.put("jobs.justlanded.com", new ParserJobsJustlanded());
/*46*/
...
//        executor.execute(new TaskStartParser((JPanel) linkPanel.getComponents()[1], new ParserF6s(), "jobs.remotive.io", getParserApp()));
        ...
        String path = PhantomJSStarter.getLibPath();
        path = path + "/lib/";


        System.out.println("PAth to png " + path);
        System.out.println("exists  " + new File(path + "up.png").exists());
        if (!new File(path + "up.png").exists())
            try {
                System.out.println("is exsists " + new File(path + "testingFile.txt").createNewFile());
            } catch (IOException e) {
                e.printStackTrace();
            }
        for (int i = 0; i < linkPanel.getComponents().length; i++) {
...
//        drupalOrgUkLabel.addMouseListener(this);
        ...
        unicornhuntLabel.addMouseListener(this);
        ...
        searchTextField.addKeyListener(new KeyAdapter() {
            ...
            public void keyTyped(KeyEvent e) {
                ...
                if (testSearchWord.length() == searchText.length() && (Character.isLetter(e.getKeyChar()) || Character.isDigit(e.getKeyChar())) || e.getKeyChar() == ' ') {
                    testSearchWord = searchText + e.getKeyChar();
                } else {
                    testSearchWord = searchText;
                }

                if (searchText.length() == testSearchWord.length() || searchText.length() == 0) {
                    testCorrectJobsList = allJobs;
                }

                System.out.println("   " + testCorrectJobsList.size() + " " + testSearchWord);
...
//            if (System.currentTimeMillis() > (new DbHelper().getDateLastUpdate() + 8 * 3600 * 1000)) {
                ...
                for (int i = 0; i < linkPanel.getComponents().length; i++) {
                    ...
                    if (homeLink.contains("smashingmagazine")) {
                        numberException = i;
                        continue;
                    }
                    executor.execute(new TaskStartParser(labelPanel, mapParsers.get(homeLink), homeLink, getParserApp()));
                }
                if (numberException != -1) {
                    jobPanel.removeAll();
                    JPanel labelPanel = (JPanel) linkPanel.getComponents()[numberException];
                    labelPanel.setBackground(new Color(0x717184));
                    JLabel label = (JLabel) labelPanel.getComponent(0);
                    String homeLink = label.getText();
                    if (homeLink.contains(" "))
...
//    private static final Logger logger = LoggerFactory.getLogger(ParserApp.class);
    ...
    private List<ExportJob> readXMS(String fileXLS) {
        ...
        try {
            ...
            path = PhantomJSStarter.getLibPath();
//            System.out.println(path);

//            FileWriter log = new FileWriter("logs.txt");
//            BufferedWriter br = new BufferedWriter(log);
//            br.write(new Date().toString() + " 1 " + path + "/lib/XLSReader.xml");
//            br.flush();
//            br.close();
//            log.close();

            inputXML = new BufferedInputStream(/*getClass().getResourceAsStream*/new FileInputStream(path + "/lib/XLSReader.xml"));

//            FileWriter log11 = new FileWriter("logs.txt");
//            BufferedWriter br1 = new BufferedWriter(log11);
//            br1.write(new Date().toString() + " 2 " + path + "/lib/XLSReader.xml");
//            br1.flush();
//            br1.close();
//            log11.close();

            XLSReader mainReader = ReaderBuilder.buildFromXML(inputXML);
            InputStream inputXLS = new BufferedInputStream(new FileInputStream(fileXLS));

//            FileWriter log2 = new FileWriter("logs.txt");
//            BufferedWriter br2 = new BufferedWriter(log2);
//            br2.write(new Date().toString() + " 3 " + path + "inputXML");
//            br2.flush();
//            br2.close();
//            log2.close();
            List<ExportJob> jobsInforms = new ArrayList<>();
            Map beans = new HashMap();
            beans.put("jobsInforms", jobsInforms);
            mainReader.read(inputXLS, beans);
//            FileWriter log3 = new FileWriter("logs.txt");
//            BufferedWriter br3 = new BufferedWriter(log3);
//            br3.write(new Date().toString() + " 3 " + path + "size = " + jobsInforms.size());
//            br3.flush();
//            br3.close();
//            log3.close();
            System.out.println(jobsInforms.size());
            return jobsInforms;
        } catch (Exception e) {
            try {
//                FileWriter log = new FileWriter("logs.txt");
//                BufferedWriter br = new BufferedWriter(log);
//                br.write(new Date().toString() + " Error " + e.getMessage());
//                br.close();
//                log.close();
                e.printStackTrace();
            } catch (Exception e1) {
    ...
    private List<ExportJob> createListForExport(List<ExportJob> selectedOffers, String fileName) {
        File f = new File(fileName);
        if (f.exists()) {
            selectedOffers.addAll(readXMS(fileName));
    ...
    private void writeExcel(List<ExportJob> exportJobs, String fileName) {
        ...
        try (OutputStream os1 = new FileOutputStream(fileName)) {
            ...
            exporter.gridExport(headers, exportJobs, "companyName,  jobTitle, jobLink, location, postingDate, sourceName", os1);
    ...
    private void formFoundJobsList() {
        List<JobsInformForSearch> correctJobsList = new ArrayList<JobsInformForSearch>();
        String searchText = searchTextField.getText().toLowerCase();
        List<String> keyList = new ArrayList<String>();
    ...
    public void panelFiller(final List<JobsInformForSearch> jobsInformList, String homeLink) {
//        Collections.sort(jobsInformList, new SortDate());
        ...
        for (int i = 0; i < jobsInformList.size(); i++) {
            JobsInformForSearch ji = jobsInformList.get(i);
            String stringDate = ji.getPublishedDate().getTime() > 1970 ? new SimpleDateFormat("dd-MM-yyyy").format(ji.getPublishedDate()) : "";
...
//
        ...
        exportButton.setBackground(new Color(0xB4B49A));
    ...
    private void onClickExportButton(List<JobsInformForSearch> jobsInformList) {
        List<JobsInformForSearch> exportList = new ArrayList<>();
        for (int i = 0; i < jobsInformList.size(); i++) {
        ...
        if (exportList.size() > 0) {
            ...
            String fileName = "OffersExport" + stringDate + ".xls";
            List<ExportJob> exportJobs = exportList.stream().map(ExportJob::new).collect(Collectors.toList());
            writeExcel(createListForExport(exportJobs, fileName), fileName);
        }
    }
...
// DO NOT EDIT OR ADD ANY CODE HERE!
    ...
    private void $$$setupUI$$$() {
        ...
        panelMain.setLayout(new GridLayoutManager(3, 2, new Insets(0, 0, 0, 0), -1, 30));
        ...
        panelMain.setMaximumSize(new Dimension(900, 712));
        panelMain.setMinimumSize(new Dimension(900, 712));
        panelMain.setPreferredSize(new Dimension(900, 712));
        panelMain.setVisible(true);
        panelMain.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(), "ParserApp", TitledBorder.CENTER, TitledBorder.DEFAULT_POSITION, this.$$$getFont$$$(null, -1, 16, panelMain.getFont()), new Color(-16777216)));
        jobPanel = new JPanel();
        ...
        Font jobPanelFont = this.$$$getFont$$$("Times New Roman", -1, 12, jobPanel.getFont());
        if (jobPanelFont != null) jobPanel.setFont(jobPanelFont);
        jobPanel.setInheritsPopupMenu(true);
        jobPanel.setVisible(true);
        panelMain.add(jobPanel, new GridConstraints(2, 1, 1, 1, GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_NONE, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_WANT_GROW, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_WANT_GROW, new Dimension(600, 500), new Dimension(600, 530), new Dimension(600, 530), 0, false));
        jobPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(), "Jobs", TitledBorder.CENTER, TitledBorder.ABOVE_TOP, this.$$$getFont$$$("DialogInput", Font.BOLD, 18, jobPanel.getFont()), new Color(-16777216)));
        jobLabel = new JLabel();
        jobLabel.setAutoscrolls(false);
        jobLabel.setEnabled(true);
        jobLabel.setFocusable(false);
        Font jobLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, jobLabel.getFont());
        if (jobLabelFont != null) jobLabel.setFont(jobLabelFont);
        ...
        backButton.setBackground(new Color(-2960187));
        backButton.setBorderPainted(true);
        backButton.setContentAreaFilled(true);
        backButton.setForeground(new Color(-11579315));
        backButton.setHideActionText(false);
        backButton.setHorizontalAlignment(0);
        backButton.setHorizontalTextPosition(0);
        backButton.setLabel("Back");
        backButton.setMaximumSize(new Dimension(100, 36));
        backButton.setMinimumSize(new Dimension(100, 36));
        backButton.setPreferredSize(new Dimension(100, 36));
        backButton.setRequestFocusEnabled(true);
        backButton.setText("Back");
        backButton.setVisible(false);
        panel1.add(backButton);
        openSearchInSelected = new JButton();
        openSearchInSelected.setAutoscrolls(false);
        openSearchInSelected.setBackground(new Color(-8937619));
        openSearchInSelected.setBorderPainted(true);
        openSearchInSelected.setForeground(new Color(-13490857));
        openSearchInSelected.setMaximumSize(new Dimension(281, 36));
        openSearchInSelected.setMinimumSize(new Dimension(281, 36));
        openSearchInSelected.setPreferredSize(new Dimension(281, 36));
        ...
        openWorkSearchButton.setLabel("Open Global Work Search");
        ...
        openWorkSearchButton.setText("Open Global Work Search");
        openWorkSearchButton.setVisible(true);
        panel1.add(openWorkSearchButton);
        searchTextField = new JTextField();
        Font searchTextFieldFont = this.$$$getFont$$$("Times New Roman", -1, 20, searchTextField.getFont());
        ...
        reparseButton.setBorderPainted(true);
        ...
        openCountrySearchButton = new JButton();
        openCountrySearchButton.setBackground(new Color(-2960187));
        openCountrySearchButton.setBorderPainted(true);
        openCountrySearchButton.setContentAreaFilled(true);
        openCountrySearchButton.setForeground(new Color(-11579315));
        openCountrySearchButton.setHideActionText(false);
        openCountrySearchButton.setHorizontalAlignment(0);
        openCountrySearchButton.setHorizontalTextPosition(0);
        openCountrySearchButton.setLabel("Open Global Country Search");
        openCountrySearchButton.setMaximumSize(new Dimension(300, 36));
        openCountrySearchButton.setMinimumSize(new Dimension(300, 36));
        openCountrySearchButton.setPreferredSize(new Dimension(300, 36));
        openCountrySearchButton.setText("Open Global Country Search");
        openCountrySearchButton.setVisible(true);
        panel1.add(openCountrySearchButton);
        openCompanySearchButton = new JButton();
        openCompanySearchButton.setBackground(new Color(-2960187));
        openCompanySearchButton.setBorderPainted(true);
        openCompanySearchButton.setContentAreaFilled(true);
        openCompanySearchButton.setForeground(new Color(-11579315));
        openCompanySearchButton.setHideActionText(false);
        openCompanySearchButton.setHorizontalAlignment(0);
        openCompanySearchButton.setHorizontalTextPosition(0);
        openCompanySearchButton.setLabel("Open Global Company Search");
        openCompanySearchButton.setMaximumSize(new Dimension(300, 36));
        openCompanySearchButton.setMinimumSize(new Dimension(300, 36));
        openCompanySearchButton.setPreferredSize(new Dimension(300, 36));
        openCompanySearchButton.setText("Open Global Company Search");
        openCompanySearchButton.setVisible(true);
        panel1.add(openCompanySearchButton);
        linkScrollPane = new JScrollPane();
        linkScrollPane.setHorizontalScrollBarPolicy(31);
        panelMain.add(linkScrollPane, new GridConstraints(2, 0, 1, 1, GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_BOTH, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_WANT_GROW, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_WANT_GROW, new Dimension(280, 530), new Dimension(280, 530), new Dimension(280, 530), 0, false));
        ...
        Font linkPanelFont = this.$$$getFont$$$("Times New Roman", -1, 12, linkPanel.getFont());
        if (linkPanelFont != null) linkPanel.setFont(linkPanelFont);
        linkPanel.setMaximumSize(new Dimension(280, 1670));
        linkPanel.setMinimumSize(new Dimension(280, 1670));
        linkPanel.setPreferredSize(new Dimension(280, 1670));
        linkScrollPane.setViewportView(linkPanel);
        linkPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(), "Links", TitledBorder.CENTER, TitledBorder.ABOVE_TOP, this.$$$getFont$$$("DialogInput", Font.BOLD, 18, linkPanel.getFont()), new Color(-16777216)));
        wfhLabelPanel = new JPanel();
        wfhLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        wfhLabelPanel.setMaximumSize(new Dimension(240, 30));
        wfhLabelPanel.setMinimumSize(new Dimension(240, 30));
        wfhLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font wfhLinkFont = this.$$$getFont$$$("Times New Roman", -1, 12, wfhLink.getFont());
        if (wfhLinkFont != null) wfhLink.setFont(wfhLinkFont);
        ...
        remoteokLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        remoteokLabelPanel.setMaximumSize(new Dimension(240, 30));
        remoteokLabelPanel.setMinimumSize(new Dimension(240, 30));
        remoteokLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font remoteokLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, remoteokLabel.getFont());
        if (remoteokLabelFont != null) remoteokLabel.setFont(remoteokLabelFont);
        ...
        landingJobsPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        landingJobsPanel.setMaximumSize(new Dimension(240, 30));
        landingJobsPanel.setMinimumSize(new Dimension(240, 30));
        landingJobsPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font landingJobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, landingJobsLabel.getFont());
        if (landingJobsLabelFont != null) landingJobsLabel.setFont(landingJobsLabelFont);
        ...
        startusLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        startusLabelPanel.setMaximumSize(new Dimension(240, 30));
        startusLabelPanel.setMinimumSize(new Dimension(240, 30));
        startusLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font startusLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, startusLabel.getFont());
        if (startusLabelFont != null) startusLabel.setFont(startusLabelFont);
        ...
        virtualvocationsLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        virtualvocationsLabelPanel.setMaximumSize(new Dimension(240, 30));
        virtualvocationsLabelPanel.setMinimumSize(new Dimension(240, 30));
        virtualvocationsLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font virtualvocationsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, virtualvocationsLabel.getFont());
        if (virtualvocationsLabelFont != null) virtualvocationsLabel.setFont(virtualvocationsLabelFont);
        ...
        simplyhiredLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        simplyhiredLabelPanel.setMaximumSize(new Dimension(240, 30));
        simplyhiredLabelPanel.setMinimumSize(new Dimension(240, 30));
        simplyhiredLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font simplyhiredLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, simplyhiredLabel.getFont());
        if (simplyhiredLabelFont != null) simplyhiredLabel.setFont(simplyhiredLabelFont);
        ...
        stackoverflowLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        stackoverflowLabelPanel.setMaximumSize(new Dimension(240, 30));
        stackoverflowLabelPanel.setMinimumSize(new Dimension(240, 30));
        stackoverflowLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font stackoverflowLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, stackoverflowLabel.getFont());
        if (stackoverflowLabelFont != null) stackoverflowLabel.setFont(stackoverflowLabelFont);
        ...
        jujuLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        jujuLabelPanel.setMaximumSize(new Dimension(240, 30));
        jujuLabelPanel.setMinimumSize(new Dimension(240, 30));
        jujuLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font jujuLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, jujuLabel.getFont());
        if (jujuLabelFont != null) jujuLabel.setFont(jujuLabelFont);
        ...
        drupalLubelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        drupalLubelPanel.setMaximumSize(new Dimension(240, 30));
        drupalLubelPanel.setMinimumSize(new Dimension(240, 30));
        drupalLubelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font drupalLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, drupalLabel.getFont());
        if (drupalLabelFont != null) drupalLabel.setFont(drupalLabelFont);
        ...
        dutchstartupjobsLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        dutchstartupjobsLabelPanel.setMaximumSize(new Dimension(240, 30));
        dutchstartupjobsLabelPanel.setMinimumSize(new Dimension(240, 30));
        dutchstartupjobsLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font dutchstartupjobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, dutchstartupjobsLabel.getFont());
        if (dutchstartupjobsLabelFont != null) dutchstartupjobsLabel.setFont(dutchstartupjobsLabelFont);
        ...
        monsterDeLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        monsterDeLabelPanel.setMaximumSize(new Dimension(240, 30));
        monsterDeLabelPanel.setMinimumSize(new Dimension(240, 30));
        monsterDeLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font monsterDeLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, monsterDeLabel.getFont());
        if (monsterDeLabelFont != null) monsterDeLabel.setFont(monsterDeLabelFont);
        ...
        weloveangularLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        weloveangularLabelPanel.setMaximumSize(new Dimension(240, 30));
        weloveangularLabelPanel.setMinimumSize(new Dimension(240, 30));
        weloveangularLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font weloveangularLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, weloveangularLabel.getFont());
        if (weloveangularLabelFont != null) weloveangularLabel.setFont(weloveangularLabelFont);
        ...
        weworkremotelyLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        weworkremotelyLabelPanel.setMaximumSize(new Dimension(240, 30));
        weworkremotelyLabelPanel.setMinimumSize(new Dimension(240, 30));
        weworkremotelyLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font weworkremotelyLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, weworkremotelyLabel.getFont());
        if (weworkremotelyLabelFont != null) weworkremotelyLabel.setFont(weworkremotelyLabelFont);
        ...
        startupjobsLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        startupjobsLabelPanel.setMaximumSize(new Dimension(240, 30));
        startupjobsLabelPanel.setMinimumSize(new Dimension(240, 30));
        startupjobsLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font startupjobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, startupjobsLabel.getFont());
        if (startupjobsLabelFont != null) startupjobsLabel.setFont(startupjobsLabelFont);
        ...
        berlinstartupjobsLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        berlinstartupjobsLabelPanel.setMaximumSize(new Dimension(240, 30));
        berlinstartupjobsLabelPanel.setMinimumSize(new Dimension(240, 30));
        berlinstartupjobsLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font berlinstartupjobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, berlinstartupjobsLabel.getFont());
        if (berlinstartupjobsLabelFont != null) berlinstartupjobsLabel.setFont(berlinstartupjobsLabelFont);
        ...
        jobsChLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        jobsChLabelPanel.setMaximumSize(new Dimension(240, 30));
        jobsChLabelPanel.setMinimumSize(new Dimension(240, 30));
        jobsChLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font jobsChLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, jobsChLabel.getFont());
        if (jobsChLabelFont != null) jobsChLabel.setFont(jobsChLabelFont);
        ...
        flexjobslabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        flexjobslabelPanel.setMaximumSize(new Dimension(240, 30));
        flexjobslabelPanel.setMinimumSize(new Dimension(240, 30));
        flexjobslabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font flexjobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, flexjobsLabel.getFont());
        if (flexjobsLabelFont != null) flexjobsLabel.setFont(flexjobsLabelFont);
        ...
        builtinnodeLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        builtinnodeLabelPanel.setMaximumSize(new Dimension(240, 30));
        builtinnodeLabelPanel.setMinimumSize(new Dimension(240, 30));
        builtinnodeLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font builtinnodeLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, builtinnodeLabel.getFont());
        if (builtinnodeLabelFont != null) builtinnodeLabel.setFont(builtinnodeLabelFont);
        ...
        weworkmeteorLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        weworkmeteorLabelPanel.setMaximumSize(new Dimension(240, 30));
        weworkmeteorLabelPanel.setMinimumSize(new Dimension(240, 30));
        weworkmeteorLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font weworkmeteorLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, weworkmeteorLabel.getFont());
        if (weworkmeteorLabelFont != null) weworkmeteorLabel.setFont(weworkmeteorLabelFont);
        ...
        jobbankLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        jobbankLabelPanel.setMaximumSize(new Dimension(240, 30));
        jobbankLabelPanel.setMinimumSize(new Dimension(240, 30));
        jobbankLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font jobbankLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, jobbankLabel.getFont());
        if (jobbankLabelFont != null) jobbankLabel.setFont(jobbankLabelFont);
        ...
        workingnomadsLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        workingnomadsLabelPanel.setMaximumSize(new Dimension(240, 30));
        workingnomadsLabelPanel.setMinimumSize(new Dimension(240, 30));
        workingnomadsLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font workingnomadsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, workingnomadsLabel.getFont());
        if (workingnomadsLabelFont != null) workingnomadsLabel.setFont(workingnomadsLabelFont);
        ...
        remoteLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        remoteLabelPanel.setMaximumSize(new Dimension(240, 30));
        remoteLabelPanel.setMinimumSize(new Dimension(240, 30));
        remoteLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font remoteLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, remoteLabel.getFont());
        if (remoteLabelFont != null) remoteLabel.setFont(remoteLabelFont);
        ...
        randstadLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        randstadLabelPanel.setMaximumSize(new Dimension(240, 30));
        randstadLabelPanel.setMinimumSize(new Dimension(240, 30));
        randstadLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font randstadLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, randstadLabel.getFont());
        if (randstadLabelFont != null) randstadLabel.setFont(randstadLabelFont);
        ...
        workopolisLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        workopolisLabelPanel.setMaximumSize(new Dimension(240, 30));
        workopolisLabelPanel.setMinimumSize(new Dimension(240, 30));
        workopolisLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font workopolisLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, workopolisLabel.getFont());
        if (workopolisLabelFont != null) workopolisLabel.setFont(workopolisLabelFont);
        ...
        elutaLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        elutaLabelPanel.setMaximumSize(new Dimension(240, 30));
        elutaLabelPanel.setMinimumSize(new Dimension(240, 30));
        elutaLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font elutaLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, elutaLabel.getFont());
        if (elutaLabelFont != null) elutaLabel.setFont(elutaLabelFont);
        ...
        jobspressoLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        jobspressoLabelPanel.setMaximumSize(new Dimension(240, 30));
        jobspressoLabelPanel.setMinimumSize(new Dimension(240, 30));
        jobspressoLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font jobspressoLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, jobspressoLabel.getFont());
        if (jobspressoLabelFont != null) jobspressoLabel.setFont(jobspressoLabelFont);
        ...
        webbjobbLabelPrser.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        webbjobbLabelPrser.setMaximumSize(new Dimension(240, 30));
        webbjobbLabelPrser.setMinimumSize(new Dimension(240, 30));
        webbjobbLabelPrser.setPreferredSize(new Dimension(240, 30));
        ...
        Font webbjobbLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, webbjobbLabel.getFont());
        if (webbjobbLabelFont != null) webbjobbLabel.setFont(webbjobbLabelFont);
        ...
        jobsSmashingmagazineLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        jobsSmashingmagazineLabelPanel.setMaximumSize(new Dimension(240, 30));
        jobsSmashingmagazineLabelPanel.setMinimumSize(new Dimension(240, 30));
        jobsSmashingmagazineLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font jobsSmashingmagazineLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, jobsSmashingmagazineLabel.getFont());
        if (jobsSmashingmagazineLabelFont != null) jobsSmashingmagazineLabel.setFont(jobsSmashingmagazineLabelFont);
        ...
        themuseLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        themuseLabelPanel.setMaximumSize(new Dimension(240, 30));
        themuseLabelPanel.setMinimumSize(new Dimension(240, 30));
        themuseLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font themuseLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, themuseLabel.getFont());
        if (themuseLabelFont != null) themuseLabel.setFont(themuseLabelFont);
        ...
        techjobsLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        techjobsLabelPanel.setMaximumSize(new Dimension(240, 30));
        techjobsLabelPanel.setMinimumSize(new Dimension(240, 30));
        techjobsLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font techjobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, techjobsLabel.getFont());
        if (techjobsLabelFont != null) techjobsLabel.setFont(techjobsLabelFont);
        ...
        webentwicklerJobsLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        webentwicklerJobsLabelPanel.setMaximumSize(new Dimension(240, 30));
        webentwicklerJobsLabelPanel.setMinimumSize(new Dimension(240, 30));
        webentwicklerJobsLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font webentwicklerJobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, webentwicklerJobsLabel.getFont());
        if (webentwicklerJobsLabelFont != null) webentwicklerJobsLabel.setFont(webentwicklerJobsLabelFont);
        ...
        uberjobsLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        uberjobsLabelPanel.setMaximumSize(new Dimension(240, 30));
        uberjobsLabelPanel.setMinimumSize(new Dimension(240, 30));
        uberjobsLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font uberjobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, uberjobsLabel.getFont());
        if (uberjobsLabelFont != null) uberjobsLabel.setFont(uberjobsLabelFont);
        ...
        guruLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        guruLabelPanel.setMaximumSize(new Dimension(240, 30));
        guruLabelPanel.setMinimumSize(new Dimension(240, 30));
        guruLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font guruLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, guruLabel.getFont());
        if (guruLabelFont != null) guruLabel.setFont(guruLabelFont);
        ...
        authenticjobsLabelParser.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        authenticjobsLabelParser.setMaximumSize(new Dimension(240, 30));
        authenticjobsLabelParser.setMinimumSize(new Dimension(240, 30));
        authenticjobsLabelParser.setPreferredSize(new Dimension(240, 30));
        ...
        Font authenticjobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, authenticjobsLabel.getFont());
        if (authenticjobsLabelFont != null) authenticjobsLabel.setFont(authenticjobsLabelFont);
        ...
        eurojobsLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        eurojobsLabelPanel.setMaximumSize(new Dimension(240, 30));
        eurojobsLabelPanel.setMinimumSize(new Dimension(240, 30));
        eurojobsLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font eurojobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, eurojobsLabel.getFont());
        if (eurojobsLabelFont != null) eurojobsLabel.setFont(eurojobsLabelFont);
        ...
        technojobsCoUkLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        technojobsCoUkLabelPanel.setMaximumSize(new Dimension(240, 30));
        technojobsCoUkLabelPanel.setMinimumSize(new Dimension(240, 30));
        technojobsCoUkLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font technojobsCoUaLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, technojobsCoUaLabel.getFont());
        if (technojobsCoUaLabelFont != null) technojobsCoUaLabel.setFont(technojobsCoUaLabelFont);
        ...
        canadajobsLabelParser.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        canadajobsLabelParser.setMaximumSize(new Dimension(240, 30));
        canadajobsLabelParser.setMinimumSize(new Dimension(240, 30));
        canadajobsLabelParser.setPreferredSize(new Dimension(240, 30));
        ...
        Font canadajobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, canadajobsLabel.getFont());
        if (canadajobsLabelFont != null) canadajobsLabel.setFont(canadajobsLabelFont);
        ...
        ziprecruiterLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        ziprecruiterLabelPanel.setMaximumSize(new Dimension(240, 30));
        ziprecruiterLabelPanel.setMinimumSize(new Dimension(240, 30));
        ziprecruiterLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font ziprecruiterLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, ziprecruiterLabel.getFont());
        if (ziprecruiterLabelFont != null) ziprecruiterLabel.setFont(ziprecruiterLabelFont);
        ...
        drupalcenterLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        drupalcenterLabelPanel.setMaximumSize(new Dimension(240, 30));
        drupalcenterLabelPanel.setMinimumSize(new Dimension(240, 30));
        drupalcenterLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font drupalcenterLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, drupalcenterLabel.getFont());
        if (drupalcenterLabelFont != null) drupalcenterLabel.setFont(drupalcenterLabelFont);
        ...
        indeedLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        indeedLabelPanel.setMaximumSize(new Dimension(240, 30));
        indeedLabelPanel.setMinimumSize(new Dimension(240, 30));
        indeedLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font indeedLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, indeedLabel.getFont());
        if (indeedLabelFont != null) indeedLabel.setFont(indeedLabelFont);
        ...
        wowjobsLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        wowjobsLabelPanel.setMaximumSize(new Dimension(240, 30));
        wowjobsLabelPanel.setMinimumSize(new Dimension(240, 30));
        wowjobsLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font wowjobsLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, wowjobsLabel.getFont());
        if (wowjobsLabelFont != null) wowjobsLabel.setFont(wowjobsLabelFont);
        ...
        builtinaustinLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        builtinaustinLabelPanel.setMaximumSize(new Dimension(240, 30));
        builtinaustinLabelPanel.setMinimumSize(new Dimension(240, 30));
        builtinaustinLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font builtinaustinLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, builtinaustinLabel.getFont());
        if (builtinaustinLabelFont != null) builtinaustinLabel.setFont(builtinaustinLabelFont);
        ...
        betalistLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        betalistLabelPanel.setMaximumSize(new Dimension(240, 30));
        betalistLabelPanel.setMinimumSize(new Dimension(240, 30));
        betalistLabelPanel.setPreferredSize(new Dimension(240, 30));
        ...
        Font betalistLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, betalistLabel.getFont());
        if (betalistLabelFont != null) betalistLabel.setFont(betalistLabelFont);
        ...
        unicornhuntLabelPanel = new JPanel();
        unicornhuntLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        unicornhuntLabelPanel.setAlignmentX(0.0f);
        unicornhuntLabelPanel.setAlignmentY(0.0f);
        unicornhuntLabelPanel.setAutoscrolls(true);
        unicornhuntLabelPanel.setBackground(new Color(-721665));
        unicornhuntLabelPanel.setMaximumSize(new Dimension(240, 30));
        unicornhuntLabelPanel.setMinimumSize(new Dimension(240, 30));
        unicornhuntLabelPanel.setPreferredSize(new Dimension(240, 30));
        linkPanel.add(unicornhuntLabelPanel);
        unicornhuntLabelPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(), null, TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.ABOVE_TOP, null, new Color(-16777216)));
        unicornhuntLabel = new JLabel();
        unicornhuntLabel.setAutoscrolls(false);
        unicornhuntLabel.setEnabled(true);
        unicornhuntLabel.setFocusable(false);
        Font unicornhuntLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, unicornhuntLabel.getFont());
        if (unicornhuntLabelFont != null) unicornhuntLabel.setFont(unicornhuntLabelFont);
        unicornhuntLabel.setForeground(new Color(-16777216));
        unicornhuntLabel.setHorizontalAlignment(2);
        unicornhuntLabel.setHorizontalTextPosition(2);
        unicornhuntLabel.setMaximumSize(new Dimension(170, 30));
        unicornhuntLabel.setMinimumSize(new Dimension(-1, -1));
        unicornhuntLabel.setOpaque(false);
        unicornhuntLabel.setPreferredSize(new Dimension(170, 30));
        unicornhuntLabel.setText("unicornhunt.io");
        unicornhuntLabel.setToolTipText("http://unicornhunt.io");
        unicornhuntLabel.setVerifyInputWhenFocusTarget(false);
        unicornhuntLabel.putClientProperty("html.disable", Boolean.TRUE);
        unicornhuntLabelPanel.add(unicornhuntLabel);
        jobsJustlandedLabelPanel = new JPanel();
        jobsJustlandedLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        jobsJustlandedLabelPanel.setAlignmentX(0.0f);
        jobsJustlandedLabelPanel.setAlignmentY(0.0f);
        jobsJustlandedLabelPanel.setAutoscrolls(true);
        jobsJustlandedLabelPanel.setBackground(new Color(-721665));
        jobsJustlandedLabelPanel.setMaximumSize(new Dimension(240, 30));
        jobsJustlandedLabelPanel.setMinimumSize(new Dimension(240, 30));
        jobsJustlandedLabelPanel.setPreferredSize(new Dimension(240, 30));
        linkPanel.add(jobsJustlandedLabelPanel);
        jobsJustlandedLabelPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(), null, TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.ABOVE_TOP, null, new Color(-16777216)));
        jobsJustlandedLabel = new JLabel();
        jobsJustlandedLabel.setAutoscrolls(false);
        jobsJustlandedLabel.setEnabled(true);
        jobsJustlandedLabel.setFocusable(false);
        Font jobsJustlandedLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, jobsJustlandedLabel.getFont());
        if (jobsJustlandedLabelFont != null) jobsJustlandedLabel.setFont(jobsJustlandedLabelFont);
        jobsJustlandedLabel.setForeground(new Color(-16777216));
        jobsJustlandedLabel.setHorizontalAlignment(2);
        jobsJustlandedLabel.setHorizontalTextPosition(2);
        jobsJustlandedLabel.setMaximumSize(new Dimension(170, 30));
        jobsJustlandedLabel.setMinimumSize(new Dimension(-1, -1));
        jobsJustlandedLabel.setOpaque(false);
        jobsJustlandedLabel.setPreferredSize(new Dimension(170, 30));
        jobsJustlandedLabel.setText("jobs.justlanded.com");
        jobsJustlandedLabel.setToolTipText("http://jobs.justlanded.com");
        jobsJustlandedLabel.setVerifyInputWhenFocusTarget(false);
        jobsJustlandedLabel.putClientProperty("html.disable", Boolean.TRUE);
        jobsJustlandedLabelPanel.add(jobsJustlandedLabel);
        jobsRemotiveLabelPanel = new JPanel();
        jobsRemotiveLabelPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 8, 0));
        jobsRemotiveLabelPanel.setAlignmentX(0.0f);
        jobsRemotiveLabelPanel.setAlignmentY(0.0f);
        jobsRemotiveLabelPanel.setAutoscrolls(true);
        jobsRemotiveLabelPanel.setBackground(new Color(-721665));
        jobsRemotiveLabelPanel.setMaximumSize(new Dimension(240, 30));
        jobsRemotiveLabelPanel.setMinimumSize(new Dimension(240, 30));
        jobsRemotiveLabelPanel.setPreferredSize(new Dimension(240, 30));
        linkPanel.add(jobsRemotiveLabelPanel);
        jobsRemotiveLabelPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(), null, TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.ABOVE_TOP, null, new Color(-16777216)));
        jobsRemotiveLabel = new JLabel();
        jobsRemotiveLabel.setAutoscrolls(false);
        jobsRemotiveLabel.setEnabled(true);
        jobsRemotiveLabel.setFocusable(false);
        Font jobsRemotiveLabelFont = this.$$$getFont$$$("Times New Roman", -1, 12, jobsRemotiveLabel.getFont());
        if (jobsRemotiveLabelFont != null) jobsRemotiveLabel.setFont(jobsRemotiveLabelFont);
        jobsRemotiveLabel.setForeground(new Color(-16777216));
        jobsRemotiveLabel.setHorizontalAlignment(2);
        jobsRemotiveLabel.setHorizontalTextPosition(2);
        jobsRemotiveLabel.setMaximumSize(new Dimension(170, 30));
        jobsRemotiveLabel.setMinimumSize(new Dimension(-1, -1));
        jobsRemotiveLabel.setOpaque(false);
        jobsRemotiveLabel.setPreferredSize(new Dimension(170, 30));
        jobsRemotiveLabel.setText("jobs.remotive.io");
        jobsRemotiveLabel.setToolTipText("http://jobs.remotive.io/");
        jobsRemotiveLabel.setVerifyInputWhenFocusTarget(false);
        jobsRemotiveLabel.putClientProperty("html.disable", Boolean.TRUE);
        jobsRemotiveLabelPanel.add(jobsRemotiveLabel);
        final JPanel panel2 = new JPanel();
        panel2.setLayout(new FlowLayout(FlowLayout.RIGHT, 0, 0));
        panel2.setBackground(new Color(-7631989));
        panelMain.add(panel2, new GridConstraints(1, 1, 1, 1, GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_BOTH, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW, null, new Dimension(300, 60), null, 0, false));
        startGlobalParsing = new JButton();
        startGlobalParsing.setBackground(new Color(-2960187));
        startGlobalParsing.setBorderPainted(true);
        startGlobalParsing.setContentAreaFilled(true);
        startGlobalParsing.setForeground(new Color(-11579315));
        startGlobalParsing.setHideActionText(false);
        startGlobalParsing.setHorizontalAlignment(0);
        startGlobalParsing.setHorizontalTextPosition(0);
        startGlobalParsing.setLabel("Start Global Reparsing");
        startGlobalParsing.setMargin(new Insets(0, 14, 0, 14));
        startGlobalParsing.setMaximumSize(new Dimension(300, 36));
        startGlobalParsing.setMinimumSize(new Dimension(300, 36));
        startGlobalParsing.setPreferredSize(new Dimension(300, 36));
        startGlobalParsing.setText("Start Global Reparsing");
        startGlobalParsing.setVisible(true);
        panel2.add(startGlobalParsing);
    }
