...
//
//            Elements tables2 = doc.select("#jobs .job");
...
//                    doc = Jsoup.parse(data.toString());
                Elements tables1 = doc.select("#jobs .job");
...
//                    System.out.println("text date : " + datePublished);
            objectGenerator(tables2.get(i).select(".location").first(), tables2.get(i).select("h4").first(),
                    tables2.get(i).select(".company").first(), datePublished, tables2.get(i).select("h4 a").last());
...
//                System.out.println("text place : " + place.text());
            ...
            jobsInform.setPlace("");
            jobsInform.setPublicationLink(startLink + linkDescription.attr("href").replaceFirst("/jobs", ""));
            jobsInform = getDescription(startLink + linkDescription.attr("href").replaceFirst("/jobs", ""), jobsInform);
...
//                    .get();
        ...
        Document document = ParserLandingJobs.renderPage(linkToDescription);
            ...
            Elements tablesDescription = document.select(".job-description").first().children();
            Elements tablesHead = document.select(".job-heading");
